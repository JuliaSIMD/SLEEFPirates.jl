...
@inline vifelse(u::Unsigned, v1::Double, v2::Double) = Double(vifelse(u, v1.hi, v2.hi), vifelse(u, v1.lo, v2.lo))
@inline vifelse(u::Mask, v1::Double, v2::Double) = Double(vifelse(u.u, v1.hi, v2.hi), vifelse(u.u, v1.lo, v2.lo))
...
# @inline trunclo(x::VecProduct) = trunclo(Vec(extract_data(x)))
@inline function trunclo(x::Vec{N,Float64}) where {N}
    reinterpret(Vec{N,Float64}, reinterpret(Vec{N,UInt64}, x) & vbroadcast(Vec{N,UInt64}, 0xffff_ffff_f800_0000)) # clear lower 27 bits (leave upper 26 bits)
end
@inline function trunclo(x::Vec{N,Float32}) where {N}
    reinterpret(Vec{N,Float32}, reinterpret(Vec{N,UInt32}, x) & vbroadcast(Vec{N,UInt32}, 0xffff_f000)) # clear lowest 12 bits (leave upper 12 bits)
end
...
@inline function dadd2(x::vIEEEFloat, y::vIEEEFloat)
    s = x + y
    v = evsub(s, x)
    Double(s, evsub(x, evsub(s, v)) + evsub(y, v))
end

@inline function dadd2(x::vIEEEFloat, y::Double{<:vIEEEFloat})
    s = x + y.hi
    v = s - x
    Double(s, evsub(x, evsub(s, v)) + evsub(y.hi, v) + y.lo)
end
...
@inline function dadd2(x::Double{<:vIEEEFloat}, y::Double{<:vIEEEFloat})
    ...
    smv = evsub(s, v)
    yhimv = evsub(y.hi, v)
    Double(s, evsub(x.hi, smv) + (yhimv) + x.lo + y.lo)
end
...
@inline function dsub2(x::Double{<:vIEEEFloat}, y::vIEEEFloat)
    s = x.hi - y
    v = s - x.hi
    Double(s, evsub(x.hi, evsub(s, v)) + evsub(-y, v) + x.lo)
end

@inline function dsub2(x::Double{<:vIEEEFloat}, y::Double{<:vIEEEFloat})
    s = x.hi - y.hi
    v = s - x.hi
    Double(s, evsub(x.hi, evsub(s, v)) + evsub(-y.hi, v) + x.lo - y.lo)
end

@inline function vifelse(b::Vec{N,Bool}, x::Double{T1}, y::Double{T2}) where {N,T<:Union{Float32,Float64},T1<:Union{T,Vec{N,T}},T2<:Union{T,Vec{N,T}}}
    V = Vec{N,T}
...
if FMA_FAST
    ...
    @inline function ddiv(x::Double{<:vIEEEFloat}, y::Double{<:vIEEEFloat})
        invy = 1 / y.hi
        zhi = x.hi * invy
        Double(zhi, evmul((fma(-zhi, y.hi, x.hi) + fma(-zhi, y.lo, x.lo)), invy))
    end

    @inline function ddiv(x::vIEEEFloat, y::vIEEEFloat)
        ry = 1 / y
        r = x * ry
        Double(r, evmul(vfnmadd(r, y, x), ry))
        # Double(r, evmul(fma(-r, y, x), ry))
    end
